...
DEFAULT_PROMPTS = {
    "timed": (
        ...
        "输出规范（必须严格遵守）：\n"
        "- 只输出新闻正文本身，使用简体中文；\n"
        "- 严禁输出思考过程、推理步骤、任务说明、提示词内容或任何解释性文字；\n"
        "- 严禁出现「作为 AI 助手」「按照要求」「以下是」等与新闻内容无关的表述；\n"
        "- 不要用 Markdown 代码块包裹正文，正文就是最终展示的新闻内容。"
    ),
    "recommend": (
    ...
    "brief": (
        "你是一位资深新闻编辑，擅长提炼要点、撰写简洁有力的每日简报。所有输出一律使用简体中文。\n\n"
        "工作方式：\n"
